...
def get_date_key(conn,date_value):
    ...
    try:
        if date_value is not None:
            print(f"SELECT date_key FROM rewards_db.dim_date WHERE date='{date_value}'")
...
def get_user_id(conn,user_id):
    ...
    try:
        if user_id is not None:
            print(f"SELECT id FROM rewards_db.dim_users WHERE id='{user_id}'")
...
def get_brand_id(conn,barcode):
    ...
    try:
        if barcode is not None and barcode!='511111704140':
            print(f"SELECT id FROM rewards_db.dim_brands WHERE barcode='{barcode}'")
...
def upsert_receipts_table(conn):
    ...
    FROM ( SELECT * FROM rewards_db.receipts_staging WHERE user_id not like 'User ID %') temp
...
def upsert_rewards_receipt_item_table(conn):
    ...
    SELECT id,receipt_id,brand_id,description,final_price, 
           ...
           user_flagged_barcode,user_flagged_new_item, 
           user_flagged_price,user_flagged_quantity
    FROM (SELECT * FROM rewards_db.rewards_receipt_item_staging WHERE brand_id NOT LIKE 'BrandID %' ) temp
    ON DUPLICATE KEY UPDATE
...
process_receipts_file("../../../receipts.json")